import { t as _sfc_main$4 } from "./AuthLayout-BCZkOr_K.js";
import { Link, useForm } from "@inertiajs/vue3";
import { ssrIncludeBooleanAttr, ssrInterpolate, ssrLooseContain, ssrRenderAttr, ssrRenderAttrs, ssrRenderClass, ssrRenderComponent, ssrRenderDynamicModel, ssrRenderStyle } from "vue/server-renderer";
import { computed, createTextVNode, mergeModels, mergeProps, onMounted, reactive, ref, toDisplayString, unref, useModel, useSSRContext, withCtx } from "vue";
...
var _sfc_main$3 = {
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "w-full" }, _attrs))}><div class="${ssrRenderClass([__props.readonly ? __props.selectable ? "brightness-70 cursor-text [&_input]:cursor-text [&_label]:cursor-text" : "brightness-70 cursor-not-allowed select-none [&_input]:cursor-not-allowed [&_label]:cursor-not-allowed" : "", "input-group text"])}"><input${ssrRenderAttr("id", __props.id ?? __props.name)}${ssrRenderDynamicModel(__props.type, model.value, null)}${ssrRenderAttr("type", __props.type)}${ssrRenderAttr("name", __props.name)}${ssrRenderAttr("autocomplete", __props.autocomplete)}${ssrIncludeBooleanAttr(__props.autofocus) ? " autofocus" : ""} placeholder=""${ssrIncludeBooleanAttr(__props.required) ? " required" : ""}${ssrIncludeBooleanAttr(__props.readonly) ? " readonly" : ""}><label${ssrRenderAttr("for", __props.id ?? __props.name)}>${ssrInterpolate(__props.label)}${ssrInterpolate(__props.required ? "*" : "")}</label></div>`);
...
var _sfc_main = /* @__PURE__ */ Object.assign({ layout: _sfc_main$4 }, {
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<div class="input-group checkbox flex items-center justify-center">`);
			...
			_push(`</div>`);
			if (unref(form).errors.auth) _push(`<p class="text-sm text-red-400 -mt-4">${ssrInterpolate(unref(form).errors.auth)}</p>`);
			else _push(`<!---->`);
			_push(ssrRenderComponent(unref(Link), {
				href: __props.resetPasswordUrl,
				class: "link link-secondary -mt-4"
			}, {
			...
			_push(`<button class="btn btn-primary" type="submit"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""}>${ssrInterpolate(__props.trans.login_button)}</button></form>`);
...
//# sourceMappingURL=Login-DWLhAELw.js.map